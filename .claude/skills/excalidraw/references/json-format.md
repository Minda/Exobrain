...
Diamond arrow connections are fundamentally broken in raw Excalidraw JSON due to rendering issues with vertices and edge offset calculations. Use styled rectangles instead for visual distinction:
...
The `label` property is for the JavaScript API, NOT raw JSON files. Use separate shape and text elements instead.
